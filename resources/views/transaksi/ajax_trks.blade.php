<script>
    // ajax batal peminjaman
    $('body').on('click', '#btn-batal-trks', function() {
        ...
        Swal.fire({
            ...
            text: "ingin membatalkan peminjaman ini!",
            icon: 'warning',
    ...
    $('#pengembalian').find('#id_dsiswa').on('change', function() {
        ...
        if (id_usr == '') {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('#pengembalian').find('#trks_tgl_pengembalian').val(new Date().toLocaleString(
                        'sv-SE', {
                            timeZone: 'Asia/Jakarta'
                        }).slice(0, 16));
                    $('#pengembalian').find('#id_dbuku').empty();
    ...
    $('body').on('click', '.modalShowTrks', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#showTrks').find('#tgl_peminjaman').html(response.trks_tgl_peminjaman.slice(0,
                    16));
                $('#showTrks').find('#tgl_jatuh_tempo').html(response.trks_tgl_jatuh_tempo.slice(0,
                    16));
                $('#showTrks').find('#tgl_pengembalian').html(response.trks_tgl_pengembalian ==
                    null ?
